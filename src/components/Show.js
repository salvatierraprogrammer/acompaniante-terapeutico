...
import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import {Collection, getDocs, getDoc, deleteDoc, collection, doc} from 'firebase/firestore';
import { db } from '../firebaseConfg/firebase';
import Swal from 'sweetalert2';
import withReactContent from 'sweetalert2-react-content';
...
const Show = () => {
    ...
    const getProducts = async () =>  {
        ...
        console.log(products);
         ...
         //4 - Funcion Eliminar documento
         const deleteProduct = async (id) =>{
            const productDoc = doc(db, 'usuarios', id)
            await deleteDoc(productDoc)
            getProducts()
         }
    //5 - Confirmacion de alert
    const confirmDelete = (id) =>{
        Swal.fire({
                    ...
                    <tbody>
                        {products.map((product) => (
                            <tr key={product.id}>
                                ...
                                <td>
                                    <Link to={`/edit/${product.id}`} className='btn btn-light'><i className="fa-regular fa-pen-to-square"></i></Link>
